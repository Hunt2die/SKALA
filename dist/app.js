...
async function runScan(value) {
  ...
  try {
    ...
    feedback('Checked '+data.domain+' from the SKALA probe. Results reflect this moment and location.');
    toast('Live check complete · '+data.summary.title.toLowerCase());
  } catch(error) {
...
function detail(key) {
  ...
  if(key==='dns') return {title:'DNS evidence',html:'<p>Queried names and resolver outcomes are shown separately. A resolver error does not mean a record is missing.</p>'+table(['Query','Outcome'],report.dns.queries.map(q=>[q.type+' '+q.name,q.state==='error'?'Unknown: '+q.error:q.state]))+'<h3 class="detail-subheading">Observed records</h3>'+table(['Type / name','Value / TTL'],report.dns.records.map(r=>[r.type+' '+r.name,r.value+' · '+r.ttl+'s']))};
